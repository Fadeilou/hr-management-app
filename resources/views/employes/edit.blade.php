...
@section('content')
    <h1 class="text-2xl font-semibold mb-6">Modifier l'Employé : {{ $employe->name }}</h1>
    ...
    <form action="{{ route('employes.update', $employe) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" required value="{{ old('name', $employe->name) }}"
        ...
        <div>
            ...
            <input type="email" name="email" id="email" required value="{{ old('email', $employe->email) }}"
        ...
        <div>
            ...
            <input type="text" name="position" id="position" required value="{{ old('position', $employe->position) }}"
        ...
        <div>
            ...
            <input type="text" name="department" id="department" required value="{{ old('department', $employe->department) }}"
         ...
         <div>
            ...
            <input type="date" name="hire_date" id="hire_date" required value="{{ old('hire_date', $employe->hire_date->format('Y-m-d')) }}"
         ...
         <div>
            ...
            <input type="number" step="0.01" name="salary" id="salary" value="{{ old('salary', $employe->salary) }}"
         ...
         <div>
            ...
            <select name="status" id="status" required
                ...
                <option value="en_conge" {{ old('status', $employe->status) == 'en_conge' ? 'selected' : '' }}>En Congé</option>
                <option value="licencie" {{ old('status', $employe->status) == 'licencie' ? 'selected' : '' }}>Licencié</option>
            </select>
             @error('status')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
